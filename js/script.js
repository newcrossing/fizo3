$(document).ready(function () {
    ...
    $('#sex').change(function () {
        ...
        if ($(this).val() == '0') {
            ...
            $("body").attr('style', ' background-color:#ffced6!important;')
        } else if ($(this).val() == 1) {
            // выбран мужчина
            $("[forsex='w']").hide()
            $("[forsex='m']").show()
    ...
    $('[numupr]').change(function () {
        ...
        request.done(function (msg) {
            ...
            $('#upr_ball_' + numupr).html(json.ball + ' баллa(ов) ').removeClass(arrColor.join(' ')).addClass(json.color)
            $('#recomend_ball_' + numupr).html(json.ball)
            $('#upr_help_' + numupr).html(json.text).removeClass(arrColor.join(' ')).addClass(json.color)
            $('#recomend_text_' + numupr).html(json.text2)
            $('#recomend_color_' + numupr).removeClass(arrColor.join(' ')).addClass(json.color)
